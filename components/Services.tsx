...
const Services: React.FC = () => {
  ...
  return (
    <section id="services" className="py-32 bg-[#F5F7F7]">
      <div className="w-full px-6 md:px-12 lg:px-16">
        <div className="text-center mb-24">
          <h2 className="text-4xl md:text-5xl lg:text-7xl font-black text-gray-900 mb-6 tracking-tighter uppercase">OUR SERVICES</h2>
          <div className="w-20 h-1.5 bg-primary mx-auto mb-6 rounded-full"></div>
          <p className="text-gray-500 font-bold tracking-[0.4em] text-xs md:text-sm uppercase">PREMIUM DENTAL SOLUTIONS FOR YOUR PERFECT SMILE</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <div
              key={index}
              className="group relative bg-white p-12 rounded-[40px] shadow-[0_10px_50px_rgba(0,0,0,0.04)] hover:shadow-[0_20px_70px_rgba(0,0,0,0.08)] hover:-translate-y-2 transition-all duration-500 border border-gray-50 overflow-hidden"
            >
              ...
              <div className="relative z-10 flex flex-col items-center text-center">
                <div className="w-24 h-24 bg-[#F0F4F4] group-hover:bg-primary rounded-[32px] flex items-center justify-center mb-10 text-primary group-hover:text-white transition-all duration-500 transform group-hover:rotate-6 shadow-sm">
                  {service.icon}
                </div>

                <h3 className="text-xl font-black text-gray-900 tracking-tight leading-tight mb-2 group-hover:text-primary transition-colors duration-300">
                  {service.title}
                </h3>

                <div className="w-0 group-hover:w-16 h-1.5 bg-primary/20 transition-all duration-500 mt-6 rounded-full"></div>
              </div>

              {/* Subtle Corner Badge */}
              <div className="absolute top-8 right-8 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <img src="/Toothfix Logo.png" alt="logo" className="w-6 h-6 object-contain grayscale opacity-30" />
              </div>
            </div>
          ))}
        </div>
